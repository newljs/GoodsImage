...
def taobao_gimg(goodshtml_text,goodshtml,skuId,pathdate):
    goods_titles = goodshtml_text.xpath('//title/text()')  # xpath截取内容
    goodsshowurls = re.findall(".*location.protocol==='http:'(.*)'//desc.alicdn.com.*", goodshtml.text)  # 通过re截取无规则html中“desc: '”//到“',”的内容
    goods_imgs_it = goodshtml_text.xpath('//ul[@class="tb-thumb tb-clearfix"]/li/div/a/img/@data-src')#xpath截取内容
    for goods_title in goods_titles:  # 遍历商品名称列表
        goods_title=goods_title.replace(' ','')
        if goods_title:
            print('正在抓取淘宝商品：%s' % goods_title)
            print('链接：%s' % skuId)
            time.sleep(1)
            ...
            for goodsshowurl in goodsshowurls:  # 遍历商品详情图列表
                goodsshowurl=goodsshowurl.replace(' ', '').replace('"', '').replace(':', '').replace('?', '').replace("'", '')
                ...
                showdescimgurls = s.xpath('//p/img/@src')  # xpath截取内容
                num = 1
            ...
            for goods_imgs in goods_imgs_it:  # 遍历商品封面图列表
                goods_imgs=goods_imgs.replace('_50x50.jpg','')
