...
def jiuxian_gimg(goodshtml_text,skuId,pathdate):
    goods_titles = goodshtml_text.xpath('//title/text()')  # xpath截取内容
    goodsshowurls = goodshtml_text.xpath('//div[@class="infoImg"]//img/@src')#xpath截取内容
    goods_imgs_it = goodshtml_text.xpath('//div[@class="show-list-con"]/ul/li/a/img/@src')#xpath截取内容
    for goods_title in goods_titles:  # 遍历商品名称列表
        print('正在抓取酒仙网商品：%s' % goods_title)
        print('链接：%s' % skuId)
        time.sleep(1)
        goods_title = goods_title.replace('/', ' ').replace('*', 'x').replace('?', ' ').replace(':', ' ').replace('\\',
                                                                                                                  ' ').replace(
            '<', ' ').replace('>', ' ').replace('|', ' ').replace(' ', '')  # .replace:替换字符;
    ...
    for goodsshowurl in goodsshowurls:  # 遍历商品详情图列表
        showdescimgurl = requests.get(goodsshowurl)  # 通过requests.get获取网络数据
        time.sleep(1)
    ...
    for goods_imgs in goods_imgs_it:  # 遍历商品封面图列表
        goods_imgs = goods_imgs.replace('1.jpg', '6.jpg')
        goods_img_url = requests.get(goods_imgs)
